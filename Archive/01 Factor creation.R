...
all_data <- all_data %>% arrange(season, desc(total_votes)) %>% group_by(season) %>% mutate(season_BR_rank = row_number(desc(total_votes)))
all_data <- all_data %>% arrange(season, player_team, desc(total_votes)) %>% group_by(season, player_team) %>% mutate(team_BR_rank = row_number(desc(total_votes)))
all_data$season_more_than_5_BR <- ifelse(all_data$total_votes>4, 1, 0)
all_data$season_more_than_10_BR <- ifelse(all_data$total_votes>9, 1, 0)
all_data$season_more_than_15_BR <- ifelse(all_data$total_votes>14, 1, 0)
all_data$season_more_than_20_BR <- ifelse(all_data$total_votes>19, 1, 0)
#View(all_data)


# Adjust Essendon players who were banned 
all_data_ess <- 
  sqldf('SELECT A.*
        ...
        WHERE A.season IN ("2017") AND A.player_name IN ("Dyson Heppell","Cale Hooker","Michael Hurley","Tom Bellchambers")
        ')
...
all_data_br <- all_data_br[!(all_data_br$season=="2017" & all_data_br$player_name=="Dyson Heppell"),]
all_data_br <- all_data_br[!(all_data_br$season=="2017" & all_data_br$player_name=="Cale Hooker"),]
all_data_br <- all_data_br[!(all_data_br$season=="2017" & all_data_br$player_name=="Michael Hurley"),]
all_data_br <- all_data_br[!(all_data_br$season=="2017" & all_data_br$player_name=="Tom Bellchambers"),]
...
# all_data$player_name_age <- floor(age_calc(all_data$DOB,all_data$Date, units = "years"))
# View(all_data)
...
all_data <-
  sqldf('SELECT A.*
         ...
         ON A.player_name_ = B.player_name
         AND A.season = B.season + 1
...
all_data <- all_data %>% arrange(match_id, Team, desc(supercoach_score)) %>% group_by(match_id,player_team) %>% mutate(team_supercoach_scorerank = row_number(desc(supercoach_score)))
...
all_data <- all_data %>% arrange(match_id, Team, desc(afl_fantasy_score)) %>% group_by(match_id,player_team) %>% mutate(team_afl_fantasy_scorerank = row_number(desc(afl_fantasy_score)))
...
all_data <- all_data %>% arrange(match_id, Team, desc(Contested_possessions),desc(supercoach_score)) %>% group_by(match_id,player_team) %>% mutate(team_CP_rank = order(desc(Contested_possessions), desc(supercoach_score)))
...
all_data <- all_data %>% arrange(match_id, Team, desc(Uncontested_possessions),desc(supercoach_score)) %>% group_by(match_id,player_team) %>% mutate(team_UP_rank = order(desc(Uncontested_possessions), desc(supercoach_score)))
...
all_data <- all_data %>% arrange(match_id, Team, desc(Disposals), desc(supercoach_score)) %>% group_by(match_id,player_team) %>% mutate(team_D_rank = order(desc(Disposals), desc(supercoach_score)))
...
all_data <- all_data %>% arrange(match_id, Team, desc(Effective_disposals), desc(supercoach_score)) %>% group_by(match_id,player_team) %>% mutate(team_ED_rank = order(desc(Effective_disposals), desc(supercoach_score)))
...
all_data <- all_data %>% arrange(match_id, Team, desc(Kicks), desc(supercoach_score)) %>% group_by(match_id,player_team) %>% mutate(team_K_rank = order(desc(Kicks), desc(supercoach_score)))
...
all_data <- all_data %>% arrange(match_id, Team, desc(Handballs), desc(supercoach_score)) %>% group_by(match_id,player_team) %>% mutate(team_H_rank = order(desc(Handballs), desc(supercoach_score)))
...
all_data <- all_data %>% arrange(match_id, Team, desc(Marks)) %>% group_by(match_id,player_team) %>% mutate(team_M_rank = dense_rank(desc(Marks)))
...
all_data <- all_data %>% arrange(match_id, Team, desc(Goals), desc(supercoach_score)) %>% group_by(match_id,player_team) %>% mutate(team_G_rank = dense_rank(desc(Goals)))
...
player_id <- data.frame(unique(all_data$player_name))
...
target <- c("1", "2","3")
votegetters <- filter(all_data, brownlow_votes %in% target)
three_vote <- filter(all_data, brownlow_votes == "3")
unique(votegetters$brownlow_votes)
...
barplot(prop.table(table(three_vote$match_outcome)))
hist(votegetters$SC)
hist(three_vote$SC)
...
library(plyr)
summarize <- count(five_goal_games,c("season","G","match_match_outcome","brownlow_votes"))
