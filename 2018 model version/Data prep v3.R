...
pl_stats <- subset(pl_stats,Season > 2011&Season < 2018&Round_type == "Regular")
tail(pl_stats)
...
mr<-subset(results_long,Season > 2011&Season < 2018&Round.Type == "Regular")
...
saveRDS(all_data, file=paste0(model_dir,"all_data_v3.RDS")) 
all_data_restrict <- filter(all_data, Disposals > 7)
saveRDS(all_data_restrict, file=paste0(model_dir,"all_data_restrict_v3.RDS")) 
...
all_positions <- position_check %>% distinct(Season, Player, Team.x, Position.y, Team.y, DOB)
write.csv(all_positions,file=paste0(data_dir,"player_positions_all.csv"))
View(all_positions)
